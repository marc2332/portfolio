...
// 3. Walk through the whole DOM to measure and cache every Node
...
// 5. Update a Node and mark it as invalidated
freya_dom.set(root_id, NodeData {
    width: Size::Pixels(150.0), // Changed from 100.0 to 150.0!
    height: Size::Pixels(100.0)
})
torin.invalidate(root_id);

// 6. Remeasure with the update DOM and invalidated Node
torin.measure(&mut freya_dom, canvas_size);
```
...
```
\
Before measuring the nodes layouts again we must call `find_best_root` so it 
attempts to find the most suitable node from which to start measuring the tree.
\
This is important because if we happened to have a very deep tree, and the only node we changed was very deep down,
...
\
`find_best_root` will also invalidate colateral nodes affected by the manual invalidations.
This is specially common when using layout strategies like percentages, auto, flex, etc,
where the computed layout is not solely defined by the Node itself, but also by other Nodes.
