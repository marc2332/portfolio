...
### Introduction
I'll be sharing with you **Torin**, a **UI Layout library** I have been working and that I use in [**Freya**](https://github.com/marc2332/freya), a GUI library for Rust.
\
...
Let's now mutate a Node and attempt to measure the layout incrementally, so only those changed Nodes are measured.

```rust
// First, we mutate the node in the DOM with the new layout config
demo_dom.set_node(
    2,
    Node::from_size_and_direction(
        Size::Pixels(Length::new(100.0)),
...
// Here we mark as dirty the node so that Torin knows this node was actualy changed
layout.invalidate(2);

// `find_best_root` is the previous step to start measuring the layout of Nodes
// It attempts to find the most suitable node from which to start measuring the tre
// This means that if there happened to be a very deep tree, and a node very deep down changed
// Torin wouldn't start measuring from the Root Node, but most probably 
// from the very own changed Node or at least an ancestor Node close to it
// This saves a lot of compute time and thus speeds up when incremental changes are applied
// 
// `find_best_root` also marks as dirty those nodes that even though were not 
// changed in the DOM directly, they might have been affected by those who were.
...
\
In the case of the example from above, the most suitable Node was... the **Root**! No incremental measures were really made! It was the Root node because it has a `Size::Inner` size for the **height**, which means that if a direct children node changes its size, the parent size might potentially change, and thus for this reason we must remeasure from the Root.
\
And, even though its true that there was basically no incremental computation done, we didnt really have to do anything special besides calling `Torin::invalidate()` and `Torin::find_best_root()`.
...
In this case the most suitable Node was not the **Root**, but the Node `1` because it was an ancestor to all the changed nodes, which are Node `2` by direct invalidation and the Node `1` because of its `Size::Inner`, the Root was not affected by those changes as it has a fixed size of `300px` for both height and width.
