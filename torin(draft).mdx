...
In this case the most suitable Node was not the **Root** (`Node 0`), but the `Node 1` because it is an ancestor to all the changed nodes or was the tallest of them, which are `Node 2` by direct invalidation and the same `Node 1` because of its `Size::Inner`.
\
